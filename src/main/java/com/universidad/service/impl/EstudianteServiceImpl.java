...
public class EstudianteServiceImpl implements IEstudianteService { // Define la clase EstudianteServiceImpl que implementa la interfaz IEstudianteService
    ...
    public EstudianteDTO obtenerEstudiantePorId(Long id) {
        // Busca el estudiante por ID y devuelve Optional<Estudiante>
        Optional<Estudiante> estudianteOptional = estudianteRepository.findById(id);

        // Si el estudiante existe, lo convierte a DTO, sino devuelve null o lanza excepción
        return estudianteOptional.map(this::convertToDTO).orElse(null);

        // Alternativa para lanzar excepción si no se encuentra:
        // return estudianteOptional.map(this::convertToDTO)
        //      .orElseThrow(() -> new ResourceNotFoundException("Estudiante no encontrado con ID: " + id));
    }

    public void saveOrUpdate(EstudianteDTO estudiante){
        Estudiante estudiante1 = convertToEntity(estudiante);
